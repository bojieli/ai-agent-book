...
```mermaid
flowchart LR
    A["提示工程<br/>Prompt Engineering<br/>优化输入指令"] --> B["上下文工程<br/>Context Engineering<br/>管理模型能看到的所有信息"]
    B --> C["Harness 工程<br/>组织模型运行并与环境交互<br/>上下文 工具 约束 验证 纠正"]
    C --> D["Loop 工程<br/>跨轮次持续自主运转<br/>谁发现下一件事 何时验证 何时算完成"]
    D --> E["Graph 工程<br/>显式执行图<br/>节点=能力 边=路由依赖 状态沿边传递"]
```
...
```mermaid
flowchart TB
    subgraph L3["数据层（最难绕过）"]
        D["管世界最终能被改成什么样<br/>行级安全策略 约束与校验器<br/>受控视图与存储过程<br/>由受信任运行时绑定的访问上下文"]
    end
    subgraph L2["执行层"]
        E["管模型能做什么<br/>工具风险评级（低/中/高）<br/>上下文之外的机制复核：独立审查进程<br/>最小权限凭证 沙盒隔离 人在回路<br/>输出检查：PII 过滤器 输出验证"]
    end
    subgraph L1["上下文层（最易绕过）"]
        C["管模型能看到什么<br/>相关性分类器 安全分类器<br/>内容审核 基于规则的保护<br/>来源标注 指令与数据分离"]
    end
    C --> E --> D
```
