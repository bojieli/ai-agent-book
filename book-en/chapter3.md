...
>
> This experiment lays bare the strengths and weaknesses of sparse retrieval: it performs excellently on queries like technical code or names due to exact keyword matching, but it cannot understand synonymous expressions (searching for one word only matches documents containing that exact word). This contrast between strengths and weaknesses provides a solid practical foundation for introducing hybrid retrieval in the next section—specific comparison examples will be presented there.
