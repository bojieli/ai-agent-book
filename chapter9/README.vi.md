...
| [browser-use](browser-use/) | 📖 | Browser-Use là framework tự động hóa trình duyệt mạnh mẽ, cho phép LLM điều khiển trình duyệt để hoàn thành nhiệm vụ phức tạp. Hỗ trợ điền form, điều hướng trang web, trích xuất dữ liệu và nhiều ngữ cảnh khác; đây là một triển khai điển hình của tự động hóa GUI (Computer Use). |
| [claude-quickstarts](claude-quickstarts/) | 📖 | Ví dụ khởi đầu nhanh và best practice cho Claude API, bao phủ nhiều ngữ cảnh sử dụng. |
