...
> 评级对象从“工具”细化到“工具+参数”：调用时根据可逆性、权限和影响面计算风险。采用基于规则的确定性检查（路径黑白名单、正则），而非模型判断。验证时依据权限和真实状态检查结构化参数；参数中的字符串仍可能携带不可信内容。

**8. (★★) 本章的 Agent 产品表格中，所有 Agent 的动作空间都是 “开放式” 的。一个受限的动作空间（比如只能从预定义选项中选择）在什么场景下反而优于开放式？**

> 高合规、高风险、错误不可逆场景：如退款、付款，受限选项即“约束”，可以减少错误选项，但仍须校验参数、权限和执行结果。
...
> 修改方向：滑窗保留——完整保留最近若干轮思考，在窗口外按 token 预算（而非固定轮数）触发滚动压缩，产出结构化状态栏（当前目标、已确认事实、已排除路径、待办）；压缩只发生一次且位置固定，缓存重建代价是一次性的，不必每轮承担。R1 剥离：节省 token、前缀稳定且有利于缓存，并与训练分布一致（历史 CoT 从不在输入中）；但每轮都要从零开始推理，会丢失长程计划，也容易重复犯错。V4 强制回传：思路连贯，长程 Agent 任务表现更好；但 token 成本高、每轮前缀都会膨胀，且无法从非思考模式无缝切换。这些差异说明历史思考的处理需要匹配模型训练方式与 API 约定，不能把某一产品的策略推广为通用规则。
...
> 状态栏中的错误可能影响后续决策。计数、时间和工具状态尽量由代码从事件记录中生成；需要模型抽取的字段应保留来源并抽样核对。对无法确认的信息标记为待核实，并测试错误状态栏是否会诱发越权或重复执行。
...
> 固定模型、任务、采样参数和总 token 预算，分别测试原始轨迹、等长摘要、代码计算的状态栏和含错误字段的状态栏。比较任务成功率、统计准确率、信息遗漏和对错误字段的依赖。如果状态栏更好，还需区分收益来自缩短输入、提前计算，还是字段位置，不能直接推断上下文学习的全部机制。
...
> 停顿或简短进度说明可以帮助用户理解等待，但不应为了显得自然而编造结论或承诺。事实错误需要明确纠正。可以测试提前开口对打断率、矛盾陈述和用户理解的影响，再选择适合任务的表达节奏。
...
> 可分别比较统一模型和快慢分工在延迟、任务成功率、可观测性及更新成本上的表现。统一模型可能减少模块间的信息损失，分工则便于替换推理后端；两种方案都需要测试打断、状态变化和执行失败，不能仅凭架构名称预测长期胜负。
...
> 稀疏奖励与昂贵的环境交互会限制部分 RL 任务的样本效率，但 RL 并非只能使用最终成败。可比较过程奖励、教师蒸馏和环境模型，检查它们是否从同一次交互中提供了更多有效信息。开放式任务还需要解决评价信号不可靠的问题。
...
> 先按失败原因分组。若格式可解析且 `pass@k` 随采样次数上升，可试验 RL、拒绝采样或蒸馏；若主要是格式错误，先比较 SFT 与约束解码；若合理采样下几乎没有成功，且缺少领域知识或基础能力，再考虑 Mid-training。切换条件应在小规模实验中制定，并同时检查目标集收益与保留集退化。
...
> 分别替换规划者与执行者，在相同任务和预算下比较，先判断规划是否确为瓶颈。为子任务写清验收条件和依赖，允许执行者指出不可执行或冲突的计划；再检查修订是否改善整体结果。
...
> 子 Agent 发送 `target_found` 后，由管理者或独立验证器检查是否满足目标。验证通过再以幂等操作确定最终结果，并广播取消。各子 Agent 在安全点清理资源并确认退出；超时未响应的实例由运行时处理。
...
> 可以让各 Agent 先基于不同证据独立判断，再共享结论与可核查来源。用相同预算比较完全共享、部分共享与独立审查，观察正确率、重复工作和错误相关性。隔离本身并不保证多样性。
