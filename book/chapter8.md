...
本章讨论如何更新模型参数：用 Mid-training 补充领域知识与基础能力，用 SFT 学习示范，用 RL 根据交互反馈调整策略。第七章建立的任务集、环境和验证器，将在这里用于构造训练数据与奖励。

不熟悉模型训练的读者可以从四阶段介绍读起；已有训练经验的读者可以重点阅读数据合成、环境与奖励设计，以及后面的案例。

模型能力开发通常包含以下四个环节，具体项目不必全部重做。

1. **预训练（Pre-training）**：在大规模文本等数据上学习语言规律、知识与基础能力。对自回归语言模型，主要目标是预测下一个 token。
2. **Mid-training（中期训练/继续预训练）**：从已有模型出发，在目标语言、领域、代码或长上下文数据上继续训练，并混入通用数据以控制遗忘。不同团队对它与预训练的边界有不同划分。
3. **监督微调（SFT，Supervised Fine-Tuning）**：用“输入—输出”示范训练模型，学习回答方式、工具调用和任务流程。
4. **强化学习（RL，Reinforcement Learning）**：让模型尝试任务，根据奖励调整行为概率。效果取决于基础能力、探索、环境与奖励设计。

后面的实验围绕两个问题展开：SFT 与 RL 在什么条件下表现出不同的泛化能力？数据和环境中的哪些问题会限制训练效果？

- **SFT 与 RL 的泛化差异。** GeneralPoints 和 V-IRL 的对照实验中，有限示范下的 SFT 出现过拟合，RL 在分布变化的测试中迁移得更好。本章用“SFT 记忆，RL 泛化”概括这一观察，但它不是两类方法的普遍属性。
- **数据与环境的作用。** 训练前应检查语料是否覆盖所缺能力、示范是否可靠、环境与奖励是否接近部署目标。仅更换 PPO、GRPO 等优化算法，通常无法弥补这些缺口。
...
表8-1 对比四个环节的数据、优化目标与典型代价，随后逐项解释。
...
自回归语言模型预训练的主要目标是**预测下一个 token（Next Token Prediction，NTP）**。

给模型一段文本，让它预测下一个 token。例如输入“中国的首都是”，训练会提高后续正确 token 的概率。损失函数衡量预测与训练文本的差异，优化器据此调整参数。模型在大量文本上重复这一过程，逐渐学习其中的语言规律、事实关联和推理模式。

给定前文，模型会为词表中的 token 分配概率。后续几种训练方法都会改变这些概率，区别在于使用什么数据，以及怎样定义优化目标。

预训练之后，模型博学却不好用：你问它问题，它可能续写出更多问题，而不是回答——因为互联网文本里，一个问题后面常常跟着的是另一个问题。它还没学会“被提问时应该回答”这个协议。

### Mid-training 的本质：在目标分布上继续学习

通用预训练不可能覆盖所有语言、领域和能力。如果模型几乎读不懂韩语文档、不了解企业内部协议，或没有形成目标任务所需的代码与长上下文表征，只靠少量回答示范或终点奖惩，可能难以补足这些能力。Mid-training 沿用预训练的下一个 token 目标，但把数据分布收窄到目标领域，并混入一部分通用保留数据以控制遗忘。它回答的是“模型是否拥有完成任务所需的知识与基本能力”，而不是“回答要长什么样”或“哪条策略奖励最高”。
...
自回归语言模型的 SFT 通常沿用预训练的下一个 token 预测目标。主要变化在于数据组织与损失计算：

1. **数据不同。** 预训练用原始互联网文本（无结构、什么都有）；SFT 用人工精心准备的“输入—输出”对，格式统一为“用户提问 → 理想回答”。模型在这些示范上继续做“预测下一个词”，于是把“被提问时该怎么组织回答”这个协议学了进去。
2. **损失只算在“回答”上（loss masking，损失屏蔽）。** 一条 SFT 样本包含问题和标注回答两部分。我们不希望模型学“怎么提问”，只希望它学“怎么回答”，所以计算损失时把问题部分的 token 屏蔽掉，只对回答部分回传梯度。具体的损失屏蔽方式取决于数据与训练实现。
...
SFT 常用于学习格式、风格和流程，也可以学到示范包含的事实。需要吸收大量领域知识时，还应比较 Mid-training；需要随时更新并引用事实时，则应比较 RAG。

> **训练成本：LoRA 参数高效微调。** LoRA（Low-Rank Adaptation，低秩适配）冻结原始权重，训练额外的低秩矩阵，以减少梯度和优化器状态的显存开销。其效果取决于目标模块、rank、学习率和任务；冻结基座也不保证原有能力完全不受影响。相关实验[^ch8-1]可作为参数选择的起点，但应在自己的保留集上检查。部署时可以加载不同 adapter 服务不同任务；本章后续实验给出各自配置。
...
第一个前提是**格式支持**。如果输出无法解析，奖励函数可以判为失败，却难以区分不同策略的任务表现。可以先用 SFT 稳定格式与基本流程，再用 RL 优化策略。
...
实际训练常先用 SFT 建立起点，再根据奖励可靠性和探索收益决定是否加入 RL。

接下来的两节 `[可选阅读]` 补充经典 RL 与预训练背景。希望直接进入实践的读者，可以从 Mid-training 一节继续。
...
交互产生的**轨迹**记录状态、动作、奖励及后续状态。**价值函数（Value Function）**估计从当前状态出发、遵循某一策略所能获得的预期累积奖励。Agent 与环境的边界由任务建模决定；环境会受 Agent 动作影响，但其状态转移通常不由策略直接控制。
...
下面四个实验考察 SFT 对语音风格、多语言思考和输入输出映射的影响。这些任务有较明确的示范目标，适合观察模型学到了什么，以及能否迁移到新输入。

这种高效率可能以依赖训练分布为代价：在需要探索多种正确策略、或部署分布偏离示范数据的任务中，SFT 容易偏向复现示范模式，在新场景中性能下降。接下来的四个实验从不同角度展示“固化输出格式”这一过程。

SFT 数据通常来自以下几种来源：

- **人工专家示范**——便于表达领域要求，但成本高，也需要复核，适合用来定义格式与风格的 “种子数据”；
- **教师模型生成**——即合成数据，让强模型批量产出 “输入—输出” 对，过滤后再蒸馏给学生，详见实验 8-8、8-9；
- **拒绝采样**——模型自己对同一问题采样多条候选，用验证器筛出正确样本再反过来训练自己，详见实验 8-9。

这些来源可以组合使用。先定义任务分布与输出 schema，再生成候选，用规则检查与人工抽检过滤，最后去重并平衡任务配比。所需样本量取决于模型和任务，应通过留出集上的学习曲线决定是否继续扩充。
...
数据合成的关键不是数量，而是覆盖面、多样性和准确性。训练集还应按任务模板、客户或时间段去重划分，评估集应避免样本泄露；若要测跨任务泛化，还需留出未参与训练的任务类型；参考解法、隐藏测试和验证器反馈不能泄露给模型。
...
“单轮”指模型接收输入、生成输出并获得奖励，无需维护跨步骤状态。接下来的 AdaptThink 实验考察思考长度的选择；GeneralPoints 在受控设置下比较 SFT 与 RL 的泛化表现。
...
仿真环境让模型反复执行任务、接收反馈并更新策略。需要重点检查三件事：

- **仿真与部署的差异。** 固定套路的用户回复、过于干净的检索结果或不真实的报错，都可能让策略依赖部署时不存在的条件。
- **交互成本。** 真实 API 可能有速率限制和不可逆副作用，训练中通常需要沙箱、模拟接口或受控的真实交互。
- **奖励的可靠性。** 环境既要产生状态变化，也要提供可用于评价任务表现的记录。

训练前应抽查仿真轨迹与真实轨迹的差异，并在部署环境中保留独立评估。

### 造不出环境怎么办：让模型扮演环境

完整仿真环境难以构建时，可以让模型生成部分环境反馈。下面介绍工具返回值模拟和状态转移模拟两种做法。
...
模型模拟器也有偏差：过于耐心的用户或从不返回噪声的搜索引擎，可能让策略学到不适用于真实场景的行为。策略还可能利用模拟器漏洞获得高奖励。可以混合模拟与受控的真实交互，并用真实轨迹定期校准。
...
任务条件可形式化时，可以使用**可验证奖励（RLVR）**：用测试、数据库断言、状态差异或格式检查评价结果。其可靠性取决于检查是否覆盖真实要求；确定性的规则同样可能被钻空子。
...
RLVP 的关键不是“奖励越密越好”，而是能否恢复组内差异。纯结果奖励在全败组和全胜组都会产生零方差、没有梯度；违规动作通常容易检测，惩罚在各条轨迹的违规行为不同时可以恢复差异；进展奖励只有在部分进展可达时才有效。设计时应遵循四点：只惩罚具体动作，不惩罚“不够努力”；结果奖励始终保留，避免模型学会什么都不做；每个惩罚最好配一条可达的合规路径；规则必须确定、难以钻空子。如果基础策略根本不会采样合规动作，应先用少量示范把这条路径“种”出来，待合规行为稳定后再逐步减弱路径塑形。换句话说，惩罚是通常可达的那一半，进展奖励是受可达性门控的那一半。
...
“稠密信号胜过稀疏信号”在一个纯 Agent 场景中也得到过验证。笔者和合作者曾在“时间感”任务上比较 DPO、四种 RL 与 On-Policy Distillation：前者分别受到稀疏奖励、目标错位、rollout 形状不匹配和策略崩溃的限制；换成冻结的 Qwen3-32B 教师，在学生自己的多轮轨迹上逐 token 对齐后，训练平滑收敛，四种条件下通过率比同源 SFT 基线高出 23 到 47 个百分点[^ch8-11]。在这个任务中，增加监督密度比继续尝试这些奖励配方更有效。

### 没有更强的教师怎么办：On-Policy 自蒸馏

在轨蒸馏需要教师在目标状态上提供有用的监督，但未必需要一个整体能力更强的外部模型。如果现有模型都不擅长目标领域，可以尝试给教师侧提供额外信息。

**On-Policy Self-Distillation（OPSD，在轨自蒸馏）**[^ch8-15]让同一模型承担教师和学生两个角色。教师侧可见标准答案或已验证解答等“特权信息”；学生侧只看问题，并在自己采样的轨迹上学习教师的逐 token 分布。额外信息能否带来有效监督，需要在留出任务上验证。
...
这一节回到第七章留下的问题：基于生产问题案例构建的评估数据集，如何真正变成后训练的输入。第七章建立的评估环境和验证器可以继续用于构造训练信号。失败归因记录、端到端回归任务、轨迹前缀回归任务、Rubric 评分各自对应不同的训练用法：
...
**评估：同时检查边界集与保留集。** 训练后的验证使用第七章的评估数据集：轨迹前缀边界集检查 “任务未完成时，模型是否选择继续验证而非宣称完成”；同样重要的是**保留集**——任务确实已完成时，模型应正常宣称完成。只盯前一个指标，会把模型训练成永远不敢收尾的**过度矫正**状态：每个任务都无限验证下去，延迟与成本崩溃。这与第七章反复强调的 “改动不能破坏既有行为” 是同一原则在参数层面的版本；评估还应抽查通用能力，确认 LoRA 补丁没有破坏其他能力。
...
开始训练前，应先检查失败来自知识不足、格式不稳，还是策略选择。再用小规模实验验证数据与奖励是否能改善这些问题。下面列出实践中容易忽略的条件。
...
6. **忽视仿真保真度**——若仿真过于简化（客服总按固定模式回复）或环境响应不真实（错误信息与生产环境不一致），训练出的策略可能难以迁移到真实场景。高保真仿真环境的构建成本可能高于训练本身。
...
Mid-training、SFT 与 RL 的用途有重叠，但训练数据和目标不同。选择方法时，应结合失败归因、格式解析率、`pass@k` 和部分进展率，检查模型缺少什么，以及当前数据能提供什么信号。“SFT 记忆、RL 泛化”只概括本章部分对照实验的结果。

训练效果还受环境保真度、验证器覆盖和数据隔离影响。可以用模型模拟承担部分交互，但应在真实或独立环境中检查迁移效果。

在轨蒸馏提供逐 token 监督，路径奖励补充终点分数之外的信息。这些方法都需要有效的训练信号；同时应检查采样与训练的数值一致性，避免策略陈旧或实现差异破坏优化。
