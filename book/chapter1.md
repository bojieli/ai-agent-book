...
这些产品都能根据任务选择步骤、调用工具，并根据结果调整后续行动。接下来，我们用 LLM、上下文和工具三个组件解释它们的工作方式。

> **阅读提示**：本章先介绍全书会用到的概念，再用实验观察 Agent 的运行。初读时不必记住所有术语，后续章节会分别展开。
...
**Manus：组合搜索、代码与浏览器操作。** Deep Research（深度调研）、Coding（代码生成）和 Computer Use（电脑操控）可以放进同一个 Agent。Manus 展示了这种组合：Agent 用浏览器获取信息，在文件系统中保存资料，再通过代码和命令行处理数据。同一项任务因此可以跨越搜索、分析和网页操作几个步骤。
...
这些系统能根据反馈持续交互，也能在行动前规划。它们的动作参数可以包含自然语言或代码，因此可表达的操作很多；实际能执行什么，仍受工具接口和权限限制。

### 工具：Agent 的手脚

工具提供 Agent 读取和改变外部环境的接口。感知类工具返回观察，执行类工具接收行动。
...
接口定义含糊容易导致误用；错误处理缺失可能让 Agent 反复重试；权限过宽则会扩大错误影响。第四章会讨论这些设计问题，以及 MCP（Model Context Protocol，模型上下文协议）如何支持工具接入。

**工具调用**（Tool Calling，也称 Function Calling）让模型用结构化请求指定工具和参数，再由框架执行。本书后续统一使用“工具调用”这一术语。
...
大语言模型（Large Language Model, LLM）负责解释请求、规划步骤和选择工具。用户的要求可能不完整，模型需要结合上下文判断是否要补充信息；执行时则根据工具结果决定下一步。预训练提供的知识和后训练形成的行为策略共同支持这些决策。

在行动之前，LLM Agent 可以先进行**内部思考**，例如分解问题、比较方案或检查计算。这些推演不改变外部环境，也可能出错，后续仍需用工具结果验证。预训练提供语言和领域知识，后训练则可以改善模型使用这些知识进行推理和决策的方式。

#### 模型即 Agent：当模型本身成为产品

“模型即 Agent”（Model as Agent）强调模型通过后训练学习工具调用策略：何时调用、选哪个工具、传什么参数。框架仍负责实际执行和结果回传。在 Agent 中，**Harness** 指围绕模型的运行设施，包括上下文管理、工具接口、安全约束、验证与纠正。这个词原指马具，本书借用它描述把模型能力用于任务执行的外围系统。

随着模型可执行的操作增多，权限、验证和错误恢复也需要相应调整。能同时改进模型与 Harness 的团队，可以根据实际运行问题优化两者的配合。

如果模型持续变强，哪些 Harness 还需要保留？Rich Sutton 在《苦涩的教训》（The Bitter Lesson）中强调了搜索与学习这类可随算力扩展的通用方法的长期价值[^ch1-1]。本书据此采取一个务实做法：模型暂时做不稳的，由 Harness 补上；新模型在评估中能稳定完成后，再考虑精简相应逻辑。权限隔离和业务约束则不能仅因模型变强就撤掉。
...
对于自然语言风格、感知或隐式决策策略等难以用规则完整描述的能力，可以考虑通过后训练更新**模型参数**。训练是否值得，取决于数据、成本和留出任务上的效果，第八章会展开。实践中三种更新方式可以组合：当前任务靠上下文适应，跨任务经验保存在外部产物中，经过筛选的数据再用于训练。
...
> **实验 1-1 ★★：上下文的关键作用**
>
> 通过系统性的**消融实验**（Ablation Study），我们探索了不同上下文组件对 Agent 行为的影响。实验从上述五个部分中选取了四个组件进行测试——本实验固定系统提示词，以保持身份和任务规则一致；去掉系统提示词的影响可另设对照研究。如图1-3 所示，五组对照实验包括：一组保留全部组件的完整基线，再加上四组各缺失一个组件的对照，以此观察每个组件对 Agent 性能的影响。
>
...
>
> 这个实验提醒我们：**上下文决定了 Agent 能看到什么，而 Agent 只能基于它看到的信息做决策**。但组件并不等价，衡量的标准是它承载的信息能否从别处重建——“每个组件都不可或缺”这类论断要靠实测，而且模型迭代很快，同一个消融换到新模型上完全可能得出不同的结论。还有一点对工程实践更重要：**“给出了回答”不等于“完成了任务”**，上下文残缺时典型的失败不是报错退出，而是一个看上去毫无破绽的答案。
...
Agent 执行任务时积累的消息历史称为**轨迹**（trajectory），包括用户消息、模型回复和工具结果。在下面的多币种收入汇总例子中，每次模型调用都接收**静态前缀 + 轨迹**：前者包含系统提示词和工具定义，后者随交互增长（图1-4）。模型的新回复和工具结果继续追加到轨迹中。
...
这个实现通过追加轨迹保留任务进展，模型下一轮能看到此前尝试和结果。开发者也可以据此检查错误发生在哪一步。积累的轨迹经过筛选和评价后，还可用于知识更新或模型训练。

下面两个实验分别观察模型如何选择搜索工具、处理返回结果和结束任务。
...
>
> 实验还观察了**意图澄清过程**：模型在研究前询问数据源、指标等要求，以减少需求歧义。以“搜索最近一个月的比特币走势，做技术分析”为例，它会先问：“你偏好使用哪个数据源？需要分析哪些技术指标？”通过这种交互式的意图澄清，GPT-5.6 能够生成更精准、更符合用户需求的研究报告。
>
> GPT-5.6 是“模型即 Agent”概念的一个成熟实例：网络搜索、代码解释器作为 Responses API 的内置工具在服务端闭环执行，客户端不必再自行搭建“搜索—阅读—分析”的编排框架。澄清问题能帮助确认需求，但不能保证消除所有歧义。
>
...
ReAct 循环能把任务运行起来，却不能保证结果正确。模型可能编造工具或参数、选错工具，或在失败后不断重试。Harness 需要在这些地方提供检查、约束和恢复机制。

图1-1 中，Model 负责决策，Harness 负责构造上下文和连接工具。生产系统还需要三项职责：**约束**限定可执行的操作，**验证**检查结果，**纠正**处理错误。
...
以 Claude Code 这类 Coding Agent 为例，文件读写、命令执行和搜索之外，还需要以下保障机制：
...
**提示工程**（Prompt Engineering）优化输入给模型的指令。

**上下文工程**（Context Engineering）管理模型接收的全部信息，包括指令、工具定义、对话历史和外部知识。

**Harness 工程**还处理工具执行、约束、验证与错误恢复。
...
这些概念有重叠，不必把它们理解为严格的五个历史阶段或嵌套层级。例如，一个执行图可以同时包含 Agent 循环、确定性程序和人工审批。选用哪种组织方式，要看任务的依赖、状态和恢复要求。

LangChain 在 Terminal Bench 2.0（评估 Agent 终端任务能力的基准）上的实践提供了一个例子：保持模型不变，通过执行结果自检、重复循环检测和思考策略调整等 Harness 改动，得分从 52.8% 提升到 66.5%。这说明外围实现能明显影响该任务集上的表现，但具体收益仍需在自己的任务上测量。
...
选型应先在自己的任务上比较成功率、延迟和成本，再决定是否值得继续优化提示词或框架。
...
**开源模型。** DeepSeek、Kimi、GLM 等模型提供了私有化部署和微调的选择，适合需要控制数据流向或定制行为的场景。成本取决于模型规模、硬件、并发和运维，不能仅凭“开源”判断更便宜。不同模型的工具调用能力也不同，选型前应在具体任务上测试。
...
**多步任务应重点评估推理能力。** 工具选择、失败恢复和动态规划都依赖推理。支持思考的模型通常值得作为候选，但额外推理也会增加延迟和成本。对分类、单步调用或固定位置点击等简单任务，可以与不启用思考的模型做对照。
...
工作流便于**控制步骤顺序**：开发者可以用代码强制检查身份、付款状态等条件，减少模型跳过步骤的机会。它也让权限和异常处理的位置更明确。不过，模型输出仍可能影响分支和工具参数，错误也可能传到后续节点；固定流程本身不能保证攻击只影响单个节点。
...
如果换用能直接理解自然语言的图像生成模型，例如 Nano Banana 2、GPT-Image 2，许多需求就可以原样输入。是否保留改写步骤，应比较它对需求细节和最终图像的影响。
...
这个对照用来判断一类 Harness 是否还有必要：当模型已能直接处理输入，原先负责格式转换的适配层可能可以精简。类似地，结构化输出可以减少 JSON 修复代码，推理模型可能减少对显式推理提示的依赖。是否删除这些代码，仍要以新模型上的测试结果为准。
...
框架和接口会更新。选型时应检查它能否满足状态管理、工具调用和错误恢复的需求，以及发生问题后能否追踪到实际调用。
...
**人工干预**（Human in the loop，又称人在回路）让人接手模型无法处理的任务或审查高风险操作。部署早期，这些接管记录也能帮助团队发现失败模式、补充评估样本。频繁请求确认会增加用户负担，因此应明确触发条件。
...
**Agent = Model + Harness** 展开了原公式中的上下文与工具部分。下表将 Harness 的五项职责与相关章节对应，方便查阅。
...
第六章讨论交互的模态和时机，第七至九章讨论评估与改进，第十章讨论多个 Agent 的协作。这些问题都可能涉及 Harness 的多项职责。
...
**提议者—审核者（Proposer-Reviewer）**：产出与评判由两个不共享上下文的角色分别承担，评判方看到的是产物本身——渲染结果、测试输出、结构化的调用参数——而不是产出方的推理过程。它用于降低自审遗漏错误的风险，但独立上下文并不保证判断独立，两个角色仍可能犯相同的错误。第三章用它更新知识，第四章用它做工具调用的事前审批与事后验证（Sidecar 是它的一个只读变体），第五章的 PPT、视频与日志三个实验都以它为骨架，第七章用它评估 UI，第九章用它审核更新提案，第十章讨论了它在对等协作中的形态，以及为什么不能让同一个 Agent 自审。
...
**最小 diff + 可回滚**：每次修改尽量小、带来源、可单独回滚，而不是整体重写。它让归因成为可能——出了问题能定位到具体哪一次改动。第三章的知识更新、第五章的代码补丁、第九章的 Prompt 与程序更新都遵循这条；本章开头给出的三条更新路径（上下文内适应、外部产物更新、参数更新），有各自的版本管理和回滚成本。

## 本章小结

Agent 由 LLM、上下文和工具组成，通过 ReAct 循环接收反馈、选择行动。模型固定时，补齐任务需要的信息和操作接口，往往就能改善结果。

消融实验提醒我们：不同上下文组件的作用取决于任务，以及缺失信息能否从其他地方重建。评估应检查实际结果，不能把一份完整流畅的回答当成任务成功。

Harness 除了组织上下文和工具，还要约束操作、验证结果并处理失败。先用单次调用或工作流解决可预测的任务，执行步骤无法预先确定时，再考虑自主 Agent。

安全检查需要落实到上下文、执行和数据三个位置，并同时测试危险请求的拦截与正常任务的完成情况。下一章从模型实际接收的消息列表出发，讨论上下文工程。
...
10. ★★★ 引言提出，模型迭代后仍需考虑信息、操作和验证问题，但解决这些问题的具体工程手段可能会过时。试举一个这样的 Agent 工程手段，并说明理由。
