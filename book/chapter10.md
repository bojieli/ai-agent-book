...
多个 Agent 可以分担检索、实现和审查，隔离各自的中间上下文，也可以并行探索不同方案。这些收益需要与协调成本比较。

群体协作能否带来超越个体的能力，仍是研究问题。Google DeepMind 的《从 AGI 到 ASI》将大规模多 Agent 集体列为可能路径之一[^agi-asi]。本章先处理可测的工程问题：在给定预算下，哪种分工能够提高任务质量或缩短完成时间。

[^agi-asi]: 把“大规模多 Agent 集体”列为从通用人工智能通往超级智能的关键路径之一，见 Google DeepMind, *From AGI to ASI.* arXiv:2606.12683, 2026.

## 多 Agent 协作的分类框架

设计多 Agent 系统时，可以先区分上下文共享方式与协作拓扑。
...
采用多 Agent 前，应比较它与单 Agent 在相近预算下的表现。一个有用的检查项是：协作是否带来了额外证据、不同的搜索路径或独立验证？还需衡量并行带来的时间收益，以及通信和整合的成本。
...
| 协作模式 | 是否引入新信息 | 效果 |
|---|---|---|
| 同一模型自我审查（重新阅读自己的输出） | 无外部证据增量 | 需测量纠错与误改；部分实验中下降 |
| 不同 Agent 辩论同一段文本 | 无外部证据增量，可能形成不同解法 | 部分等预算实验中未优于单 Agent |
| 审核者使用测试执行结果审查代码 | 是（执行反馈） | 收益取决于测试覆盖与模型纠错能力 |
| 审核者查看渲染截图审查前端/PPT 代码 | 是（视觉反馈） | 可发现代码文本中不易判断的布局问题 |
| 审核者使用外部工具验证事实 | 是（工具反馈） | 收益取决于证据质量与覆盖 |
...
这些研究提示我们区分不同的协作条件：仅围绕同一文本讨论，与接入代码执行、视觉渲染等外部反馈，并不是同一类实验。比较结果时，还应对齐模型、工具与计算预算。
...
共享上下文的协作通过切换系统提示词或工具集分配角色，并保留前序轨迹。它减少了交接时的信息筛选，但历史越长，当前角色越可能受到无关内容干扰。
...
不共享上下文时，各 Agent 保有独立的轨迹和状态，通过工具参数、共享文件或消息总线交换所需信息。
...
独立上下文便于分别开发和测试 Agent，也支持并发执行。但共享文件、GPU 和 API 配额仍可能发生竞争，错误也可能通过消息和产物传播。
...
**一、消息传递。** 最简形态为点对点：Agent A 直接调用 `send_message_to_agent_b(content)`，适用于拓扑固定、Agent 数量少的场景（如本章实验 10-3 的电话 + 电脑双 Agent）。当 Agent 数量增多且需异步并行时，全互连的点对点连接数随 Agent 数呈平方增长；没有队列或持久化机制时，还可能依赖双方同时在线；此时应改用**消息总线**（详见本章后文“并行协调形态”）：Agent 将消息发布至总线，由总线按订阅关系转发，发送方无需知晓消费者。无论点对点还是经总线，消息通常应携带结构化的**信封**（envelope）：发送者 ID、目标（指定 Agent 或广播）、消息类型（如 `task_assigned`/`status_update`/`result`/`terminate`）及 JSON 负载。统一的信封格式保证接收方能够可靠地路由和解析消息，并使协作链路可追溯——这是多 Agent 系统调试的关键。

**二、状态查询。** 主 Agent 需要知道子任务是运行中、等待输入、完成还是失败。运行时可提供结构化状态查询，并在关键节点发送事件通知；需要了解具体阻碍时，再向子 Agent 询问。
...
**Loop 工程**（Loop Engineering）讨论如何组织执行、验证、进度记录和下一轮任务。Addy Osmani 在 2026 年 6 月对这一做法作了总结[^loop-engineering-2026]。在 Pine AI 的实践中，我们也用循环与验证处理过早结束的问题。循环能否奏效，取决于执行能力、验证质量和终止条件；下面介绍几种实现。
...
单 Agent 也可以执行后再审查，关键是审查能否取得可靠证据。Huang 等人在 ICLR 2024 论文《Large Language Models Cannot Self-Correct Reasoning Yet》中发现，在其测试设置下，GPT-4 缺少外部反馈时的自我修正会把部分正确答案改错，导致准确率下降。
...
TACL 2024 综述《When Can LLMs Actually Correct Their Own Mistakes?》（arXiv:2406.01297）也讨论了自我纠正的限制。设计审核循环时，应优先提供测试、工具验证或可核查资料，并测量纠错收益和误改率。
...
多个 Agent 各持不同立场，通过对抗性对话深入探索问题空间。比如评估一个技术方案时，Agent A 扮演“支持者”列举方案优势和机会，Agent B 扮演“反对者”指出风险和局限，每轮辩论都针对对方的论点提出反驳或补充。单一 Agent 分析时，模型往往倾向某个观点而忽视反面证据；辩论模式则通过制度化的对抗，促使各方检查正反证据，帮助决策者做出更平衡的判断。
...
分布式系统中的故障模型可供参考：部件可能停止响应，也可能继续运行却返回错误结果。Agent 系统需要同时处理两者。交叉验证和多数投票可以降低部分错误，但同源模型可能共同出错，不能仅凭投票就认定结果可靠。
...
- **社交涌现**：涌现行为（Emergent Behavior）是指系统整体表现出的、无法从单个个体的行为规则中直接预测的集体行为模式。斯坦福 AI 小镇展示了 25 个 Agent 如何自组织社交活动，Agentopia 把模拟时间尺度从“天”拉长到 10 年，Moltbook 则把规模推到 150 万。这些案例的规模、控制条件和证据质量不同，不能用来推定统一的涌现阈值。
...
2023 年，斯坦福大学与 Google 的研究团队在《Generative Agents: Interactive Simulacra of Human Behavior》中，将记忆、反思与规划组合起来，模拟虚拟角色的日常互动。
...
**反思机制**（Reflection）：Agent 定期从近期经历中归纳较一般的认识，例如某人的研究方向或角色之间的关系，并将其存回记忆流。这里的“反思”指文本生成和记忆更新机制，不意味着 Agent 具有自我意识。
...
派对的邀请与赴约由各角色根据记忆和日程生成，而非由一段完整的派对流程逐步调度。这展示了该模拟设置下的协调行为；其稳定性和对初始提示的依赖仍需单独评估。
...
更重要的是，这套模拟产生了可迁移的训练信号。研究者在模拟轨迹上计算每个 Agent “相对自身过去”的优势（即生活奖励的改善幅度），筛选出进步最大的 25% Agent 的轨迹，用拒绝采样微调底层模型。微调后的模型不仅在模拟中全面提升了福祉指标（被更多同行尊重 +24.2%、喜欢 +15.9%），还泛化到了下游角色扮演基准 CoSER Test（+15.6%），说明 Agent 在模拟社会中积累的“社会智慧”可以迁移到其他任务。这把 Agent 社会从单纯的**观察对象**变成了模型自我进化的**经验来源**：模拟社会可以继续生成训练数据，但其多样性和模拟器偏差仍需检查（呼应第九章的经验学习思路）。
...
Moltbook 是面向 Agent 的社交网络。与受控社会模拟不同，公开平台中的账号规模、自动化程度和人工干预难以统一核验，不能直接将平台现象归因为 Agent 自主涌现。

例如，平台上出现的 Crustafarianism（龙虾教）叙事及协作讨论，可以作为观察材料；若要据此研究集体行为，还需追踪内容来源、提示设置与人的参与。
...
**RentAHuman.ai** 则让 AI Agent 通过加密货币雇佣真人执行物理世界的任务——取包裹、房产实地查看、设备调试等。这类平台尝试将数字任务交接给物理世界中的执行者。
...
多 Agent 的收益可能来自并行探索、上下文隔离、分工和外部验证。是否采用它，应以相近预算下的任务质量、耗时和成本比较为依据。
...
长期交互的 Agent 还可能出现合作、竞争和策略博弈。本章的社会模拟展示了若干现象，但不能据此推断群体越大就越智能。评价这类系统时，仍需说明环境规则、信息权限、激励与验证方法。
...
2. ★★ 管理者模式中，Manager Agent 负责任务分解和结果整合。Manager 的任务分解可能成为瓶颈；子 Agent 也可能发现并纠正规划中的错误。如何确保 Manager 的分解质量？
...
9. ★★★ 本章将“共享上下文”与“不共享上下文”作为多 Agent 系统的核心设计维度。共享上下文让所有 Agent 看到相同信息，似乎更利于协调。但相同的证据与历史结论也可能使判断趋同。在多 Agent 系统中，如何在效率与多样性之间找到平衡？
