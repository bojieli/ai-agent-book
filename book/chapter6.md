...
用户说完一句，Agent 想一段、调用几个工具，再回一句，这是前文常用的交互方式。但它思考时，邮件可能到达，用户可能改主意，网页也可能变化。本章讨论如何接收这些更新，以及怎样把交互扩展到语音、屏幕和物理环境。
...
同步调用会等待操作返回，异步调用则允许程序在等待期间处理其他工作。例如，Agent 等待一次长查询时，仍需要接收用户补充信息。这里需要处理三件事：
...
事件驱动架构把用户消息、工具结果和外部通知送入事件队列，再由运行时安排处理（图6-1）。外部来源可以主动推送，也可能需要定时轮询；异步不要求所有来源都采用同一种接入方式。
...
第三方事件源还需要具体的接入适配。如果新邮件或 API 回调没有连接到 Gateway，Agent 就只能通过后续轮询发现变化。
...
电话中的验证码请求不能等下一次周期检查。PineClaw 的通道把事件及时送到 Agent，运行时还要保证消息归属正确，并按紧急程度处理。
...
Agent 可以使用用户账号，也可以拥有独立的邮箱、电话号码和工作环境。独立身份便于说明它代表谁、能做什么，并限制暴露的账号范围；必须访问用户账号的任务，则需要明确授权和受控的登录流程。

虚拟电脑（VM/容器）和虚拟手机（Android 模拟器）可为 Agent 提供独立工作环境，减少与用户桌面的干扰。隔离效果取决于挂载、凭证、网络和运行时配置；即使环境隔离，发送邮件等外部操作仍可能造成实际影响。
...
多个来源的事件可能同时到达。下面讨论同一个 Agent 实例如何安排处理顺序。
...
这些字段帮助模型区分用户要求和第三方内容，也便于运行时检查权限。来源标签不能单独阻止提示注入；系统还需保留可信的来源记录，并控制第三方消息能触发的操作。

Webhook、定时器、邮件、数据库变更和文件监听都可以转换成统一事件格式，再交给同一个路由层。

**基于紧急度的动态处理策略。**

事件可以等待、取消当前工作后处理，或交给独立会话并行处理。选择取决于紧急程度和任务依赖。

![图6-2 异步事件处理的三种策略](images/fig6-2.svg)

**取消式处理（Cancellation-Based）** 用于紧急事件，其本质是为紧急事件**提前制造一个安全点**：主动中断当前步骤，把这一刻变成可以消费新事件的边界。当紧急事件到达时（如用户点击 “停止” 或监督系统发来高优先级指令）：(1) 停止当前操作——如果 LLM 正在推理，立即取消流式响应；如果有同步工具在执行，发送取消信号；(2) 清空待处理队列，将所有事件取出；(3) 将队列中的事件和紧急事件一起追加到轨迹末尾；(4) 立即重新调用 LLM，以更新后的完整轨迹为输入来评估局势。例如，用户在 Agent 执行可能错误的操作时输入 “停！我说错了”，Agent 会立即看到这条新输入，重新理解真实意图，以减少后续错误操作；已经执行的副作用仍需单独确认和处理。
...
长工具调用期间如何接收打断，还取决于模型接口是否允许未完成调用继续保留在轨迹中。
...
这套方案维持同步接口要求的工具配对关系。占位符必须明确表示“未完成”，真实结果到达后再带来源和任务 ID 追加。原生异步接口则可直接保留未完成状态。
...
关键在于工具的名称和描述本身就要传达异步的语义。当模型看到 `initiate_phone_call` 时，更容易区分“发起”与“完成”。工具描述应进一步强化这一点：“此工具将启动由子 Agent 处理的电话任务。任务成功发起后立即返回任务 ID，你可以继续处理其他事项。通话结束后会收到单独的通知事件。”
...
语音不占用双手和视线，适合边工作边提出要求，也便于随时补充或打断。语音输入法把口述转为文字，语音 Agent 还负责听说之间的交接；两者都可支持引言提到的 whisper coding。
...
这里重点比较发言权如何交接：级联和轮次式系统需要判断一句话何时结束，全双工系统则持续处理听与说的重叠。
...
在理解方面，Omni 模型可以利用停顿、语调等声音信息；在生成方面，可以输出唱歌或带特定语气的语音。具体能力取决于模型训练。
...
快模型先回应，慢模型在后台推导，可以缩短首次等待。但简单问题可能被重复处理，复杂问题也可能前后矛盾：快模型先建议购买，慢模型随后发现套餐缺少关键功能。需要限制即时回应中的实质性承诺。
...
方案二让后台模型通过状态栏或专门接口提供建议，前台维持对话并组织表达。它减少了两个模型直接给出不同答案的机会，但仍需检查前台是否正确理解后台结果，以及等待期间怎样回答追问。

#### 方案三：端到端思考与表达统一

方案三把思考能力直接内化到端到端音频模型中。Step-Audio R1 用两个互补机制解决两个问题：**模态锚定思考蒸馏（MGRD）** 让模型基于声学特征思考，**MPS 双脑架构**让构思与表达并行。前者改善基于音频的推理，后者减少完整思考结束前的等待。
...
Computer Use 把观察扩展到屏幕，将动作变为点击、输入和滚动。页面可能在截图后变化，因此与语音交互一样，系统需要在行动后确认实际状态。
...
模型输出元素 ID 后，系统根据当前元素位置执行操作。这减少了直接预测坐标的负担，但仍可能漏掉非标准控件，或因页面变化使旧 ID 失效。标注与文本列表的 token 成本也需要计入。
...
三条路线的选择逻辑可以概括为：**结构化信息可得时，优先用 DOM/Accessibility Tree 索引**，减少直接预测坐标的负担；**不可得时**（原生桌面软件如 Photoshop、Canvas/WebGL 渲染的界面、游戏），**既可以用视觉标注（原始 SoM 路线），也可以用坐标预测**。视觉标注把定位变成选择题，对未经专门训练的通用模型更友好；坐标预测省去标注步骤，对做过 GUI 定位训练的模型更直接。两者在小元素和密集界面上的精度都仍有差距。
...
离散截图可能遗漏两帧之间发生的事，例如短暂通知、动画和会议声音。观察接口需要按任务捕获这些变化。
...
移动端还受应用权限、平台政策和商业合作限制。即使模型能正确操作界面，也未必获得相应访问能力。

平台限制可能涉及安全、反滥用和商业利益。Agent 代替用户完成任务，可能改变广告曝光、推荐流量和交易入口；但不能仅凭一次限制就确定平台的动机，也不能假定 Agent 不会受推荐内容影响。

因此，消费级 Computer Use 的可行性需要同时考察操作能力、平台接口与授权条件。
...
遥操作可以帮助判断自主失败来自哪里。如果人在同一硬件上能完成红杯入托盘、废纸入垃圾盒的任务，就说明硬件至少支持这些动作。自主系统失败时，可以优先检查感知、规划和控制，但这并不排除硬件误差对可靠性的影响。
...
真机遥操作提供任务能力的参照，但不便批量改变物体位置和故障条件。下面用二维模拟器和理想控制器建立可重复的对照。
...
异步运行时需要记录事件来源、任务归属和未完成状态；语音系统需要处理发言权与后台思考；Computer Use 和机器人还要在动作后重新观察。它们都需要让计划跟上环境变化。
...
唤醒、取消、抢占和恢复的具体实现随接口而异。收到停止要求不等于工具已经停止，预测成功也不能替代结果检查。

下一章讨论如何评估这些系统：既测最终任务结果，也检查过程中是否正确处理了状态变化和错误。
