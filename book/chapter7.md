...
Agent 搭好后，需要判断哪些任务能完成、哪些容易失败，以及一次修改是否带来了改善。评估要同时检查模型、工具、运行环境和结果验证器。
...
可以用**对比实验**改变一个变量并比较结果，用**消融实验**关闭某个组件以观察其贡献。两者都需要控制任务和运行条件，并考虑随机波动。

**评估对象是模型与 Harness 的组合。** 同一模型换了提示词、工具或反馈机制，表现可能改变。因此，失败分析要区分模型没有所需能力、系统没有提供所需信息，以及接口或验证器本身出错。

**模型替换实验（model swap）**固定 Harness，只更换模型。分数不涨，可能是接口缺陷，也可能是候选模型都不擅长该任务、预算不足或样本太少；分数变化则说明模型选择有影响。它能提供归因线索，不能单独证明瓶颈位置，还需结合轨迹和组件消融。

新模型在公开基准上更好，不代表在自己的业务中也更好。固定任务集可以帮助检查升级后的成功率、成本和退化；暂时达不到上线门槛的任务，也可以保留为后续模型的评估目标。
...
这条满分轨迹中还包含一处未被验证器捕获的问题。telecom 的 Agent 政策首段即规定 “You should only make one tool call at a time”，而第 4 条消息中 Agent 一次发出了 `get_customer_by_phone` 与 `get_customer_by_name` 两个调用。验证器没有因此判定出错，原因在于本题的 `reward_basis` 只考虑最终状态。这是该任务评分范围的限制，并非二元奖励必然无法检查过程。但生产环境中的评估系统往往需要更多：不仅要判定对错，还要指出问题出在哪里。
...
下面分别讨论成功标准、评估环境、任务来源和验证方法。

## 评估指标：成功的定义

上一节五条任务通过四条，样本太少，不能直接推断业务中每五个用户就有一人失败。即使成功率估计可靠，还要看失败后果：退款、探索性研究和内容草稿，对错误与重试的容忍度不同。

### 多次尝试：用 Pass@k 衡量探索能力

**Pass@k** 衡量同一任务运行 $k$ 次时至少一次通过的比例。它适合检查在给定尝试预算下能否找到成功结果。若每次输出是连续得分，可以取最好的一次，记为 **Best@k**。

长时运行的编译器开发、数学探索和漏洞研究，都可以从多次尝试中筛选成功结果。

对这类工程与科研探索，展示的通常不是“每次都做对”，而是把探索预算拉长后终于出现一条突破性轨迹。对于科研发现、漏洞挖掘、开放式创作等任务，这种能力上限本身就很有价值：人类可以从 $k$ 条候选轨迹中挑出那一条最好的。

产品演示也常突出一条成功轨迹，例如持续搜索、操作虚拟电脑并生成报告。它能说明某种能力存在，却不能替代重复运行后的成功率。

评估这类展示时，应同时询问：一共尝试了多少次、花了多少时间和 token、失败的结果是什么，以及成功样本由谁筛选。
...
其他基准也可以按任务、状态、工具、评分和执行协议这五项检查。

### 人机交互型与工具调用型评估环境

telecom 需要模拟用户参与操作与对话。代码生成、数学求解等任务则可能只与工具交互，并用测试或答案检查器评分。这不意味着它们一概无需标注或模型评判，验证方式仍取决于任务要求。
...
下面比较公开基准如何选择任务、设计验证器和防范泄漏，再讨论自建业务集的来源。
...
**SWE-bench Verified 同时检查修复和回归。** FAIL\_TO\_PASS 测试要求原先失败的用例通过，PASS\_TO\_PASS 要求原先通过的用例继续通过。两组结果分别提供修复与回归证据，但覆盖范围有限，不能证明不存在其他缺陷；测试也应由独立验证流程保护。
...
**Terminal-Bench** 的 `build-linux-kernel-qemu` 任务要求构建 Linux 内核 6.9、在 `start_kernel` 中加入自定义 printk、生成 initramfs 并在 QEMU 中运行，以启动日志中的消息作为验收依据。验证器还需要确保读取的是实际运行结果。
...
**Terminal-Bench 在题面中嵌入金丝雀标识符。** 每道题携带一个 canary GUID，若在未向模型提供该标识的测试中仍能复现相关内容，可作为数据泄漏的线索，进一步排查来源。它不阻止数据泄漏，但使泄漏可被检测。
...
**自建业务集**覆盖真实的任务分布，可以作为模型选型、Harness 设计决策的依据。例如，可以借鉴 τ²-bench 的交互结构，但还需针对业务重写状态、工具、用户行为和验证标准。

**生产轨迹回流**来自线上的真实失败用例：用户明确纠正、用户点踩，以及事后通过状态检查、规则验证器或 LLM 评审发现的问题案例，经失败归因后沉淀为回归用例。具体做法见后文 “失败归因” 与 “端到端回归任务与轨迹前缀回归任务” 两节。这类案例贴近真实使用，但仍需核查用户反馈、环境状态和归因是否可靠。

起步阶段通常只有公开基准与少量手写的自建业务集；系统上线运行一段时间后，生产轨迹回流的用例会成为主体。

## 自动化评估方法

程序化验证适合测试结果、数据库状态和精确答案等检查。它通常成本较低，规则也便于复查；但公开基准并非只用代码判定，部分自然语言条件仍可能需要模型评判。

只报告总分会丢失诊断信息。例如，0 分本身不能说明线路选择和充值步骤哪里出错。程序验证器也可以输出失败断言、步骤和状态差异，再结合轨迹分析原因。

生产场景还有另一重困难：许多判断根本无法写成代码可检查的断言。一封投诉回复是否得体，一份调研报告是否遗漏了关键信息，一次记忆检索是否弄错了人物关系，这些既没有唯一终态可供查询，也无法靠关键词匹配来判定。

图7-4 按任务的**可机械验证程度**排列几种方法，实际系统可以组合使用。

![图7-4 验证方式的谱系：从确定性验证到模型评判](images/fig7-4.svg)

难以写成确定性检查的维度，可以用 **Rubric** 明确评价条件，再由 **LLM-as-a-Judge** 打分。评分和理由仍需抽样审计，失败归因则进一步核查轨迹中的证据。
...
评判模型可能存在偏差，相同输入重复评判也可能波动。例如，**长度偏差**会让冗长回答在内容没有改善时得到更高分。可以用内容相同、长度不同的对照回答检查这一点，并审计评分与长度的关系；相关性本身不能证明偏差，还要排除内容质量差异。
...
持续针对同一个评分器优化，可能让系统适应它的盲点。因此，需要保留未用于调优的任务，并检查分数提升是否对应实际质量改善。

可以用不同家族的模型按同一 Rubric 评判，比较分歧并抽样交给人复核。不同模型仍可能共享偏差，多数一致也不能代替证据核查。日常用较低成本的评分器，定期开展多模型与人工审计，是一种可选安排。
...
Agent 成本还受轮数、工具返回长度和缓存影响，需要按完整任务统计。
...
先比较任务成功率、工具调用、延迟和成本。假设新模型在简单任务上更便宜，但多轮工具任务成功率下降了 5 个百分点，应先检查统计不确定性，再考虑按任务分流。分流本身也要测试，不能把分类正确当作前提。
...
第一轮先试最便宜的改法。假设 H1 认为 Agent 只是“不知道路”，因此只给实验组补充 Wi-Fi 设置的导航提示和最终状态检查要求。结果成功率没有变化，未支持这次提示词修改有效；仅凭这组小样本，还不能排除其他提示词改法。
...
把评估工具放进产品开发流程，可以降低重复实验的成本。下面讨论消融开关、AB 测试、提示词快照和数据收集。
...
收集数据前应确定需要哪些字段、怎样脱敏、谁可以访问，以及保留多久。类型约束可以减少误用，但不能替代对字段内容和数据流的检查。

### 从外部到内部：评估思维的转变

这些设施各有用途：消融开关比较组件贡献，AB 测试比较版本，提示词快照定位输入变化，数据约束控制采集范围。应按团队的实验需求逐步接入。

## 仿真环境：从评估到后训练的桥梁

评估环境也可以用于训练：让 Agent 多次交互，根据结果更新策略。训练通常对吞吐、重置和任务多样性提出更高要求；数字任务与机器人任务所用的仿真形式也不同。

程序化验证器可以改造成奖励函数；LLM Rubric 则提供模型评分，两者的可靠性不能混为一谈。用于训练时，还需要测试环境重置是否完整、并行实例是否相互污染，以及模型会不会利用评分漏洞。训练与最终评估应保留独立任务。
...
评估先定义成功标准，再确定任务分布、验证方式和运行预算。Pass@k 检查多次尝试中的成功机会，Pass^k 检查重复运行的可靠性；两者都需写明采样口径和失败成本。
...
发现分数变化后，先检查环境和验证器，再通过轨迹、消融和模型替换提出假设。局部实验通过，只说明值得扩大测试；上线判断还需要完整回归和成本检查。

第八章讨论如何把这些反馈用于模型训练，第九章讨论如何据此更新知识、指令和程序。
...
6. ★★ 本章区分了 Pass@k 的能力上限与 Pass^k 的业务可靠性。对于一个单次成功率只有 60% 的 Agent，怎样结合任务的失败成本、重试成本和副作用，决定应该报告哪个指标、取多大的 $k$？
