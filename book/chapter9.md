...
Agent 即使反复处理相似任务，也可能一再犯同样的错误。部署后如何从经验中学习，是本章讨论的问题。它既涉及持续学习研究中的遗忘，也涉及生产环境里的反馈识别：哪些行为有效，哪些只是偶然成功，哪些错误值得改变后续策略。
...
保存轨迹只是第一步。要形成可复用经验，还需比较成功与失败、识别适用条件，并在新任务上验证。第三章侧重用户与领域事实，本章则侧重行动策略，例如某种恢复方法适用于哪些接口版本。
...
图9-1 将前面章节的状态管理、知识库、代码工具、评估和后训练组织成一个持续进化闭环。

![图9-1 Agent 持续进化的总体闭环](images/fig9-1.svg)

本章先讨论如何评价运行轨迹，再比较知识、指令、程序和参数四种更新方式，最后说明更新如何被验证、发布与淘汰。
...
**中间的过程验证器回答“是否以允许的方式办成”。** 结果正确并不代表过程正确：删除失败的测试用例也能让测试通过，口头承诺用户 “我们会在 7 天内退款，请耐心等候” 也可能得到暂时的满意反馈。这一层检查业务规则、权限和动作序列，用以区分“结果已经达成”与“结果以允许的路径达成”。可形式化的权限和动作约束可以由代码判定；涉及政策语义或例外解释的部分仍需进一步审查。

**上层的质量验证器回答“是否让用户舒服”。** 例如，客服是否耐心、是否提供了合规范围内的变通方案，研究报告是否抓住了关键证据，生成文本是否自然简洁。这些维度补充了任务完成与规则遵从的检查，部分也会影响实际结果。此时可以使用第七章介绍的 LLM-as-a-Judge，预先定义评价量表（Rubric），要求验证器逐项给分并引用轨迹证据。
...
学习信号说明 Agent 应当改变，但没有说明改变应发生在哪里。事实和经验适合写成知识文档；可以用语言清楚表达的策略适合写入提示词或 Skill；可以精确执行的流程与约束适合写成程序；难以用外部规则表达的能力则可能需要更新模型参数。图9-3展示了这四种方式及其关系。
...
系统提示学习与第二章的提示工程不是一回事。第二章讨论怎样组织一份好 Prompt；本节讨论什么反馈足以触发修改，以及更新提案怎样安全发布。修改应是带来源的最小 diff，而不是让模型每次都重写整份 Prompt，并保留回滚版本。待验证版本必须同时在**触发失败的边界集**和**正常工作的保留集**上测试，前者要改善，后者不能退化。
...
同样的做法可以移植到其他领域。航空客服 Agent 的典型问题案例是：用户对退票费、改签费或行李政策提出异议，Agent 未查询政策、未解释规则、未寻找合规的替代方案，即调用 `transfer_to_human`。转接条件应以该业务的正式政策为准，并区分用户要求人工、权限不足和一般政策争议。诊断同样指向转接边界未予明确，修法同样是将其落成一条带出处的最小规则。
...
第一章将 DeepSeek Harness（`dsh`）列为自进化框架[^dsh-2026]。其底座 Cordis 关注组件在运行中装入、卸下和重新配置时的组合问题[^cordis-2026]。这对需要热更新的 Harness 尤其重要。
...
运行中的更新需要处理带状态的副作用与变化的依赖。若资源清理和依赖通知不完整，卸载插件可能遗留事件监听器，替换模块也可能破坏调用方；整体重启虽能简化部分问题，却可能打断任务。
...
`dsh` 将模型适配器、工具注册表、会话日志和 Agent 主循环组织为插件，使这些部分可以沿同一机制替换。可组合性降低了修改成本，但代码质量和更新验证仍需单独保证。
...
有些能力难以完整写成规则，例如语音韵律和视觉识别，改善它们往往需要参数训练。文本风格和长程规划则可能同时受提示、程序与参数影响，应通过实验选择修改位置。弯引号的作用域判断也是如此：程序可以解析部分语法边界，Skill 表达语境与例外，后训练则可强化跨任务的识别习惯。
...
Voyager[^voyager-2023] 在 Minecraft 中根据当前能力选择目标，通过环境反馈修改程序，将验证成功的程序存入技能库，再组合已有技能完成后续任务。它提供了课程生成、技能复用与验证相结合的实例。
...
**假设、实验、评估、反馈。** 更新提案应给出预期效果，再用实验决定保留还是撤回。Discovery Loop 也提出将实验设计、实现、评估与后续反馈连接起来[^ch1-discovery-loop]。对持续进化系统而言，重点是让下一轮修改依据可复查的结果。
...
部署后的推理不会自动形成可靠的长期学习。本章采用的办法是保存运行证据，离线提出更新，并在独立任务上验证后发布。
