...
Agent 要处理未预先定义的任务，往往需要临时编写代码、保存中间结果，再根据执行反馈修改。本章讨论 Coding Agent 如何支持这种工作方式。

本书重点介绍 **Coding Agent + 文件系统** 这类架构。Coding Agent 能编写、修改和执行代码；文件系统保存代码、数据、记忆和产物。Manus、OpenClaw 等产品展示了这种组合在开放任务中的用途。

代码生成还可以创造新的工具，本书因此把它称为**元能力**。例如，Agent 可以先写一个解析器，再用它处理当前文件。

代码把条件和步骤写得更明确，也便于用执行结果和测试检查推理。不过，程序能运行，并不代表它实现了正确的需求。将“年龄大于 18 且已实名认证”写成条件表达式后，仍需确认业务要的是“大于”还是“大于等于”，以及身份状态来自哪里。

## Coding Agent

### Coding 是 Agent 的基础能力

对需要计算、处理数据和创建产物的 Agent，代码生成是一项实用的基础能力。可以从一组文件与执行工具开始实现。
...
这七个工具是教学用的能力清单，主要属于第四章的感知与执行两类。具体实现可以合并接口，也可以另加协作和事件工具。
...
工具数量没有固定答案。Shell 可以完成大部分文件操作，专用读写与搜索工具则能提供更明确的参数、输出和权限边界。这里列出七种能力，是为了逐项说明它们如何配合。

这些工具可以组合出新的处理流程：用求解器计算、把规则写成校验函数，或针对新数据格式编写解析器。后文会用实验比较这些用法。

### 案例：从 Manus 到 OpenClaw——通用 Agent 的 Coding 内核

Manus、OpenClaw 等产品组合了搜索、浏览器操作和代码执行。代码在其中承担数据处理、产物生成和工具适配等工作。

例如，PPT 和 Word 文档可以通过 OOXML 或相应库生成，PDF 报告可以从 Markdown、HTML 或 LaTeX 构建，图表可以由 Python 绘制。服务提供 API 时，也可以用代码直接访问；只有界面可用时，则需要 Computer Use。选择哪条路径，要比较实现成本、权限和实际稳定性。
...
**用文件保存记忆与产物。** OpenClaw 将高层事实和用户偏好保存在 `MEMORY.md`，并使用按日期归档的 Markdown 日志。文本便于阅读、修改和版本管理，但时间顺序、来源与引用需要显式维护，格式本身不会自动解决检索冲突。
...
**适用边界。** 本章的判断主要针对调研、内容生成和数据处理等开放任务。垂直客服等任务可以围绕固定流程和领域工具实现，未必需要让模型在运行时写代码。代码生成是否值得加入，应看它能否解决现有工具难以覆盖的问题。
...
接触仓库时，Agent 应先阅读与任务相关的项目指令、文档和代码，确认构建与测试方式。需要理解的范围随修改规模变化。

关键文档缺失时，可以从代码中确认模块职责、依赖和测试入口，并记录后续工作需要的结论。简单修复无需先为整个仓库补写架构文档。
...
复杂修改可以先写简短设计：涉及哪些模块、采用什么方案、有哪些兼容性影响和验证方法。需要用户决定需求或接受重大取舍时，再提交审查；已明确授权、范围清楚的修改可直接实现。

**代码实现与测试。**

方案明确后，Agent 按项目规范实现，复用已有抽象和工具。

实现后应运行与改动相关的检查，并为新增行为或易回归的问题补充测试。失败时分析原因、修复后重跑。完成标准应包括需求是否满足和必要验证是否通过，不能只看文件是否已经写出。

测试通过后，还应阅读 diff，检查遗漏的边界条件、不必要的改动、兼容性和权限问题。静态检查和独立审查可提供补充证据，但都不能保证发现所有缺陷。
...
代码任务可以利用测试、类型检查、Linter 和版本控制提供反馈。下面讨论 Harness 如何组织这些已有设施。
...
当需求清楚且测试有效时，代码任务能较快获得自动反馈。Git 也便于审查和撤回文件改动。不过，测试可能不完整，部署和数据变更也未必能由 Git 回滚；这些边界仍需单独处理。
...
4. **回退要可靠**：Agent 在安全网内操作才能大胆试错。Git 分支和快照便于恢复文件或环境状态，但外部通信和生产数据变更需要独立的恢复方案。
...
工具、API 和上下文都可能出错。本节按故障位置讨论检测、恢复和终止方式[^ch5-3]。
...
可恢复的工具错误应作为输入返回模型。例如，工具不存在时返回名称错误，参数不合法时返回约束说明。若框架修复了参数格式，必须明确记录修复内容；会改变含义的输入不能静默改写。

短暂故障可以自动恢复，内部应保留完整记录。若恢复明显延长等待、改变执行方案或需要用户提供信息，应及时说明；最终失败时再汇总原因与已尝试的动作。
...
可以使用持久终端保留会话状态，也可以为每次调用显式指定工作目录和环境变量。前者方便接续交互，后者更便于隔离与重放；并行任务尤其需要避免共享状态相互干扰。无论采用哪种方式，都要让模型知道哪些状态会保留。

**即时的语法反馈机制。**

文件修改后，可以运行适用的语法检查或 Linter，把文件位置和错误信息返回模型，便于及时修正。检查频率应与成本匹配；大范围批量修改可先完成一组关联编辑，再统一检查。
...
可以按已知线索选择工具：知道名字时用 grep 或 glob，需要语义探索时尝试嵌入检索，需要精确追踪定义和调用时使用符号工具。不必每次都走完同一套搜索顺序。

### Coding Agent 中的文件编辑工具

编辑接口需要让模型明确指定修改位置和内容。图5-4 对比五种方案的定位方式、输出开销与常见失败。
...
Coding Agent 的安全问题包括私有数据外泄、危险命令和跨会话注入。下面先明确威胁，再讨论沙盒、命令检查和数据权限。
...
全权限本地 Agent 可能同时涉及这些风险，部署时需要检查它能读哪些资料、接收哪些外部内容，以及向哪里发送数据。
...
- **沙盒隔离与网络出口控制**——代码执行扩大了 Coding Agent 的攻击面，隔离级别与出口策略的工程选型见本节后文；
...
LLM 直接输出计算或逻辑结论时可能出错。把问题形式化后交给代码执行或求解器，可以检查其中一部分错误；形式化过程本身仍需核对。
...
本实验选用较弱模型，以观察代码辅助是否减少推理错误。换成更强模型后，纯思考可能已能解决所测谜题，辅助收益因而缩小。同一方法在不同模型上的收益需要分别测量，不能由一次对照推断所有 Harness 都会随模型变强而失去价值。

### 代码作为业务规则的约束

Agent 可以把已明确的业务规则写成校验代码，但规则解释和代码实现都要经过验证。

“购买后 7 天内可退款”仍需说明：按自然日还是工作日，起点是下单还是发货？写代码会迫使开发者选择具体含义，却不会自动选对。应先由业务方确认规则，再检查实现。
...
校验工具可以重复执行已明确的条件，处理时间计算、多条件组合和跨数据源检查。输入来源、代码实现和异常处理正确时，结果才可信。
...
**第一层：参数作为思考的 checklist**。工具描述中列出了完整的取消政策，并要求模型“调用前先查询订单详情、逐条核对”；可选的 `expected_*` 参数进一步促使模型把自己的判断依据显式写出来。为了填好这些参数，模型应先调用查询工具获取订单详情，逐一确认每个条件——填写参数的过程本质上是一个**调用前的 checklist**。当模型查到舱位是经济舱且未购保险时，很可能在准备调用的过程中就注意到规则 5，从而**根本不会发起调用**，而是直接告诉用户“该订单不符合当前取消条件，可进一步查询是否有其他政策允许的处理方式”。这一层的价值在于引导思考、减少无效调用；但它不承担安全责任——`expected_*` 参数只是模型的自我陈述，服务端从不把它当作事实。
...
这里三种做法分工不同：自然语言规则供模型解释政策，参数提示帮助调用前核对，服务端校验执行具体约束。前两者不能替代最后的校验，服务端实现也需要测试与审计。
...
报告、幻灯片和交互页面可以用代码生成：HTML 描述结构，CSS 控制样式，JavaScript 实现交互。这让 Agent 能直接修改位置、内容和样式，减少界面定位操作；实际显示效果仍需渲染检查。

**PPT 生成 Agent。**

Slidev 等框架支持用 Markdown 和 HTML 编写幻灯片，再渲染成页面。Agent 可以据此生成内容、调整布局并反复预览。
...
这里沿用第四章的提议者—审核者模式，但审核对象是渲染产物。Reviewer 每轮检查最新图片，Proposer 接收文本反馈，能减少主上下文中累积的图像。双 Agent 会增加协调成本，是否优于单 Agent 自查，应在相同预算下比较。
...
**一看产物能否用少量参数描述。** 法兰盘的外径、厚度、孔位和孔径等可以用参数化代码表达；植物、石头或人脸的外观细节则更难用少量规则描述。

**二看精度要求和验证方式。** 零件尺寸需要与规格、公差逐项核对。参数化代码便于控制这些数值，生成模型的输出也可以测量，但未必满足约束。几何检查通过后，制造仍有材料与工艺要求。

**还要看可编辑性。** CAD 流程常用 B-rep（边界表示）实体和 STEP 等交换格式，但不能假定交换文件完整保留了建模特征树。保存参数化源代码，便于修改孔径等尺寸后重新生成；网格产物也可编辑，只是按尺寸约束修改通常需要更多处理。
...
外部服务缺少 SDK 或返回格式变化时，Agent 可以阅读文档和样本，生成 HTTP 请求、解析器或数据转换代码。适配结果应检查字段含义、错误处理和版本兼容性。下面以日志解析为例。

没有 API 的系统可以先通过 Computer Use 操作，再把稳定的步骤写成 RPA 脚本。界面变化可能让脚本失效，因此仍需验证与恢复机制，第九章会讨论。
...
Agent 可以根据新样本生成解析补丁，再通过新旧格式的测试决定是否发布。
...
日志解析失败时，系统可以把样本与错误报告给 Agent，生成修复提案。在隔离环境中检查解析结果和可视化效果，并通过旧格式回归后，再按发布流程更新前端。
...
**用 HTML 交付可交互的结果。** 需要筛选数据、展开细节或演示操作时，HTML 页面比静态报告更方便。主要供阅读和版本审查的文字仍适合 Markdown。交付格式应按读者如何使用结果来选。
...
需求不完整时，需要收集缺失信息。文本可以一次提出多个问题，但选项多、存在依赖关系时，表单更便于集中填写和检查。
...
大量数据不必经过模型逐行复述。图5-9 的 **Artifact 模式**让 Agent 生成 SQL，系统执行后直接将结果渲染成表格。这样减少数据传递中的 token 开销和抄写错误，但查询是否表达了正确需求仍需验证。
...
更进一步，Agent 可以生成两个 artifact 形成流水线：SQL 查询 + 可视化代码（如柱状图）。前端将 SQL 结果直接传给可视化代码，LLM 只负责生成代码，不参与数据传递——这正让生成逻辑与数据传递分开。
...
Agent 也可以根据交互动态生成软件。Anthropic 的“Imagine with Claude”展示了这种可能性的边界：用户提出需求，Claude 实时生成前端界面和交互逻辑，用户与生成的软件交互，Claude 修改代码生成新界面展示操作结果。整个过程中用户看到一个从无到有、持续演化的应用程序。
...
生成 Agent 实现时，应检查上下文协议、工具接口和依赖版本。常见问题包括：
...
可以提供经过验证的 Agent 实现作为参考，让模型基于已有循环和接口修改。

选择与任务相近的参考实现后，调整提示词、工具和业务逻辑，再用新任务及原有回归集测试。参考代码降低了从零实现的成本，但其中的假设和缺陷也可能被继承。
...
Coding Agent 用文件与执行工具完成修改，再依靠测试、静态检查和运行反馈验证结果。故障处理还要区分可重试错误、需要修改输入的错误，以及必须停止或接管的情况。

代码生成也能支持计算、规则校验、文档制作、接口适配、交互界面和新的 Agent 实现。共同要求是把生成与验证接起来：计算要核对建模，页面要检查渲染，业务操作要由可信数据和权限机制约束。

下一章讨论异步事件、语音、屏幕和物理操作，看看 Agent 在执行期间如何接收新信息、处理打断并恢复任务。
