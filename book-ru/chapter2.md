...
С точки зрения управления контекстом механизм Skills крайне дружественен к KV Cache. Если поместить определения всех специализированных инструментов кода в системный промпт, их разрастание будет съедать огромное количество токенов, а при изменениях ещё и ломать префикс кэша; а в модели «Skill + универсальный исполнитель» число инструментов всегда остаётся небольшим (как показано в главе 5, достаточно всего семи базовых инструментов), а содержимое Skill загружается по требованию через описанный выше механизм прогрессивного раскрытия, не затрагивая уже закэшированный префикс. Подробное сравнение двух форм и framework выбора между ними даны в главе 4, а глава 8 разбирает, как агент в процессе самоэволюции выбирает, в какую из форм закреплять новую способность.
...
Ранее уже были проанализированы два мотива сжатия (контроль длины и повышение качества рассуждения) и внутренний механизм «обучение в контексте — это по сути поиск». На этой основе можно вывести четыре принципа, направляющих проектирование конкретных стратегий сжатия (в главе 8 будет рассказано, как Claude Code напрямую превращает метафору консолидации памяти в периодическую офлайн-систему интеграции памяти):
...
Общее у всех этих технологий — явное, инженерное управление знаниями: вместо того чтобы модель пассивно искала нужное в море информации, ей заранее предоставляют уже отфильтрованные структурированные знания. Вернёмся к «Горькому уроку» Ричарда Саттона: в конечном счёте побеждают универсальные методы, эффективнее использующие больше вычислительных ресурсов. Каждая технология, представленная в этой главе — от раскладки контекста, дружественной к KV Cache, до контекстно-ориентированной компрессии, — это конкретная инженерная практика максимизации эффективности использования информации в рамках текущих границ возможностей моделей. Естественное продолжение этого пути — постепенная передача агенту самой задачи проектирования структуры знаний: он сам будет самостоятельно превращать разрозненные сырые данные в динамически развивающиеся структурированные знания, сам открывать структуру мира, а не пассивно принимать заранее заданную нами структуру (это направление будет раскрыто в главе 8, «Самоэволюция агента»).
