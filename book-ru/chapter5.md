...
Что ещё важнее, способность агента писать файлы означает, что он может **самоэволюционировать** через запись файлов. Когда агент впервые выполняет некую задачу и обнаруживает ранее неизвестную ключевую информацию (например, при звонке в банк выясняется, что для подтверждения личности требуется указать адрес отделения, где открыт счёт), он записывает этот опыт в базу знаний, и при повторном выполнении той же задачи эта информация подгружается автоматически. Этот механизм «чем больше используешь, тем умнее становится» по сути является конкретной реализацией парадигмы экстернализованного обучения, которую подробно рассмотрим в восьмой главе.
...
6. **Сам агент** — создание новых агентов с помощью кода, формирующее самозагрузку (в отличие от «самоэволюции» без изменения весов, описанной в главе 8).

Читая вдоль этой линии «от внутреннего к внешнему и в конечном итоге обратно к себе», легче увидеть единую ценность кода как мета-способности. Создание новых инструментов по мере необходимости — дальнейшее развитие этой мета-способности, которое будет раскрыто в главе 8.
...
Здесь нужно провести чёткую границу с восьмой главой. В этом разделе речь о том, как агент с помощью кода **чинит и создаёт агентов, подобных себе** — самовосстановление, самокопирование, размножение новых агентов по мере необходимости; объект воздействия — код и структура агента. «Самоэволюция» из восьмой главы — это другое: там агент **без изменения весов модели** непрерывно наращивает возможности (накапливает опыт, оптимизирует промпты, собирает инструменты); объект воздействия — знания и политика агента. Оба процесса можно назвать «эволюцией», но чтобы не путать с названием восьмой главы, здесь мы называем способность «производить агентов с помощью кода» **самозарождением** (bootstrapping).
...
На этом мы завершили рассмотрение двух из трёх опор — контекста и инструментов, а генерация кода как раз является самым универсальным из инструментов. Но остался нерешённым один ключевой вопрос: как научно измерить эффективность всех этих архитектурных решений? Начиная со следующей главы мы переходим к третьей опоре — модели, и начнём с оценки. В следующей главе мы построим полную методологию оценки — от создания среды оценки, проектирования наборов данных до моделей вознаграждения и выбора моделей на основе оценки, что даст количественные средства проверки для всех технических решений, обсуждавшихся в предыдущих главах.
