...
| --- | :--: | --- |
| [use-computer-while-calling](use-computer-while-calling/) | 📖 | Implements a dual-agent collaboration architecture with a Phone Call Agent and a Computer Use Agent. The two agents communicate directly via WebSocket without a coordinator. The Phone Agent handles voice interaction, while the Computer Agent performs browser automation, working in parallel to complete complex tasks requiring both voice and web operations. |
