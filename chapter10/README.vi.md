...
| --- | :--: | --- |
| `use-computer-while-calling/` | 📖 | Triển khai kiến trúc cộng tác hai Agent gồm Agent gọi điện thoại và Agent sử dụng máy tính. Hai Agent giao tiếp trực tiếp qua WebSocket, không cần coordinator. Agent điện thoại xử lý tương tác giọng nói, Agent máy tính thực thi tự động hóa trình duyệt; hai bên làm việc song song để hoàn thành nhiệm vụ phức tạp cần cả giọng nói và thao tác web; code đã tách thành [TalkAct](https://github.com/19PINE-AI/TalkAct) độc lập, kho này không đóng gói thư mục này (lệnh clone xem phụ lục trong README chính). |
...
| `generative_agents/` | 📖 | Các Agent tạo sinh kiểu "thị trấn AI" của Stanford (dự án đi kèm Thí nghiệm 10-7); kho ngoài `joonspk-research/generative_agents`, cần tự clone (xem phụ lục trong README chính). |
